...
private:
  ...
  void EmitPrintInstruction(
      ...
      std::vector<std::vector<std::string>> &TableDrivenOperandPrinters,
      unsigned &BitsLeft, unsigned &AsmStrBits);
  void EmitGetRegisterName(raw_ostream &OS);
  void EmitPrintAliasInstruction(raw_ostream &OS);
...
PrintCases(std::vector<std::pair<std::string, AsmWriterOperand>> &OpsToPrint,
           raw_ostream &OS, bool PassSubtarget) {
...
static void EmitInstructions(std::vector<AsmWriterInst> &Insts, raw_ostream &OS,
                             bool PassSubtarget) {
  ...
  for (unsigned i = 0, e = FirstInst.Operands.size(); i != e; ++i) {
    if (i != DifferingOperand) {
      ...
      while (!OpsToPrint.empty())
        PrintCases(OpsToPrint, OS, PassSubtarget);
      OS << "    }";
    }
    OS << "\n";
  }
...
void AsmWriterEmitter::EmitGetMnemonic(
  ...
  StringTable.emitStringLiteralDef(OS, "  static const char AsmStrs[]");
...
void AsmWriterEmitter::EmitPrintInstruction(
  ...
  if (!Instructions.empty()) {
    ...
    while (!Instructions.empty())
      EmitInstructions(Instructions, OS, PassSubtarget);
...
static void
emitRegisterNameString(raw_ostream &OS, StringRef AltName,
...
void AsmWriterEmitter::EmitGetRegisterName(raw_ostream &OS) {
  const Record *AsmWriter = Target.getAsmWriter();
  StringRef ClassName = AsmWriter->getValueAsString("AsmWriterClassName");
  ...
  if (hasAltNames) {
    for (const Record *R : AltNameIndices)
      emitRegisterNameString(OS, R->getName(), Registers);
  } else
    emitRegisterNameString(OS, "", Registers);
...
void AsmWriterEmitter::EmitPrintAliasInstruction(raw_ostream &OS) {
  const Record *AsmWriter = Target.getAsmWriter();

  OS << "\n#ifdef PRINT_ALIAS_INSTR\n";
  OS << "#undef PRINT_ALIAS_INSTR\n\n";
  ...
  if (PatternsForOpcode.empty()) {
    ...
    OS << "  return false;\n";
    OS << "}\n\n";
    OS << "#endif // PRINT_ALIAS_INSTR\n";
    return;
  }

  // Forward declare the validation method if needed.
  if (!MCOpPredicates.empty())
    OS << "static bool " << Target.getName() << ClassName
  ...
  if (MCOpPredicates.empty())
    ...
    OS.indent(2) << "  &" << Target.getName() << ClassName
                << "ValidateMCOperand,\n";
  OS.indent(2) << "};\n";
  ...
  OS << "void " << Target.getName() << ClassName << "::"
...
void AsmWriterEmitter::run(raw_ostream &OS) {
  std::vector<std::vector<std::string>> TableDrivenOperandPrinters;
  unsigned BitsLeft = 0;
